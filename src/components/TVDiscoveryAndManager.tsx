...
export const TVDiscoveryAndManager: React.FC<TVDiscoveryAndManagerProps> = ({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <div>
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              ...
              Kayıtlı TV Cihazları ({devices.length})
            </h2>
            <p className="text-xs text-slate-500">
              Eşleştirilmiş Samsung televizyonları yönetin, hedef TV'yi değiştirin veya özel isim atayın.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>IP ile TV Ekle</span>
            </button>
            <button
              id="btn-ping-all-tvs"
              onClick={() => tvDeviceManager.pingAllDevices()}
              title="Çevrim içi durumu kontrol etmek için tüm TV'leri sorgula"
              className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors cursor-pointer"
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-mono focus:border-indigo-500 focus:outline-hidden"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Özel Tanımlayıcı İsim
                </label>
                <input
                  type="text"
                  placeholder="ör. Salon TV / Yatak Odası"
                  value={manualName}
                  onChange={(e) => setManualName(e.target.value)}
                  className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs focus:border-indigo-500 focus:outline-hidden"
                />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      title={device.onlineStatus ? `Durum: ${device.onlineStatus}` : 'Durum bilinmiyor'}
                    ...
                    >
                      ...
                      />
                      {device.onlineStatus === 'online' ? 'Çevrim İçi' : device.onlineStatus === 'offline' ? 'Çevrim Dışı' : 'Bilinmiyor'}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      disabled={isPinging}
                      title="TV'yi HTTP 8001 /api/v2/ üzerinden sorgula"
                    ...
                    {devices.length > 1 && (
                      <button
                        ...
                        title="TV'yi kayıtlı cihazlardan kaldır"
                  ...
                  {isEditing ? (
                    <div className="flex items-center gap-1.5">
                      <input
                        ...
                        className="flex-1 px-2.5 py-1 text-sm font-bold border border-indigo-500 rounded-lg focus:outline-hidden"
                        autoFocus
                      />
                      <button
                ...
                <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                  <div className="flex items-center gap-1 text-[11px] text-slate-500">
                    ...
                    <span>Yetki Jetonu: </span>
                    {device.token ? (
                      <span className="font-mono text-emerald-600 font-medium">Eşleşti</span>
                    ) : (
                      <span className="text-amber-600 font-medium">Eşleşmedi</span>
                    )}
                  </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div>
            <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
              ...
              Yerel Ağda TV Keşfi (Tarama)
            </h2>
            <p className="text-xs text-slate-500">
              8001 /api/v2/ tanı uç noktası üzerinden yerel Wi-Fi ağınızdaki Samsung Smart TV'leri otomatik arayın.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-3.5 bg-slate-50 border border-slate-200 rounded-2xl mb-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded-xl text-xs font-mono text-slate-900 focus:outline-hidden focus:border-indigo-500"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              disabled={discoveryState === 'SCANNING'}
              className="w-full px-3 py-1.5 bg-white border border-slate-300 rounded-xl text-xs text-slate-900 focus:outline-hidden focus:border-indigo-500"
            >
              <option value="quick">Hızlı Aralık (.50 ile .120 arası - Önerilen)</option>
              <option value="full">Tüm Alt Ağ (.1 ile .254 arası - Kapsamlı)</option>
            </select>
          </div>

          <div className="flex flex-col justify-end">
            <div className="text-[11px] text-slate-500">
              Protokol: <span className="font-mono text-slate-800">HTTP GET :8001/api/v2/</span>
            </div>
            <div className="text-[11px] text-slate-500 mt-0.5">
              Hedef: <span className="font-semibold text-slate-800">Tizen 5.5+ TU8500</span>
            </div>
          </div>
        </div>
          ...
          <div className="mb-4 p-4 bg-indigo-50/50 border border-indigo-200 rounded-2xl space-y-2 animate-fade-in">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-indigo-900 flex items-center gap-1.5">
                ...
                Ağ taranıyor: <code className="font-mono text-indigo-700">{progress.currentIp}</code>
              </span>
            ...
            <p className="text-[11px] text-indigo-600">
              Şu ana kadar {progress.foundCount} Samsung Smart TV bulundu...
            </p>
          </div>
        )}
        ...
        {discoveryState === 'COMPLETED' && (
          <div className="mb-4 p-3 bg-emerald-50 border border-emerald-200 rounded-xl text-xs text-emerald-800 flex items-center justify-between">
            <span>
              Tarama tamamlandı. Yerel ağınızda <strong>{discoveredList.length}</strong> Samsung TV bulundu.
            </span>
            <span className="text-[11px] text-emerald-600 font-mono">Durum: Tamamlandı</span>
          </div>
        )}
        ...
        {discoveredList.length > 0 ? (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[10px] text-emerald-600 flex items-center gap-1 mt-0.5">
                        ...
                        <span>Gecikme: {tv.responseTimeMs || 25}ms</span>
                      </div>
                    </div>
          ...
          discoveryState === 'COMPLETED' && (
            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-center text-xs text-slate-500">
              Bu IP aralığında yanıt veren Samsung Smart TV bulunamadı. Televizyonun açık ve aynı Wi-Fi ağına bağlı olduğundan emin olun.
            </div>
          )
        )}
      </div>
      ...
      <div className="bg-slate-900 text-slate-200 rounded-2xl p-5 border border-slate-800 shadow-xs">
        <div className="flex items-center gap-2 text-indigo-400 font-bold text-sm mb-2">
          <Terminal className="w-4 h-4" />
          <span>Fiziksel Cihaz Terminal Keşif Aracı (SSDP & CLI)</span>
        </div>
        <p className="text-xs text-slate-400 mb-3">
          Fiziksel yerel ağ entegrasyon testi için terminalinizde doğrudan bağımsız SSDP çok noktaya yayın (multicast) aracını çalıştırabilirsiniz. Bu araç, TU8500 ve diğer Tizen cihazlarını keşfetmek için UDP 1900 portu üzerinden UPnP M-SEARCH paketleri iletir:
        </p>
